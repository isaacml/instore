...
func acciones(w http.ResponseWriter, r *http.Request) {
	...
	if accion == "destinos" {
		...
		query, err := db.Query("SELECT id, padre_id FROM usuarios WHERE user = ?", r.FormValue("userAdmin"))
		if err != nil {
			Error.Println(err)
		}
		for query.Next() {
			//Obtengo el identificador del usuario admin externo
			err = query.Scan(&id_user_admin, &id_father_admin)
			if err != nil {
				Error.Println(err)
			}
		}
		...
		if id_father_admin == 0 || id_father_admin == 1 {
			...
			if internal_action == "gent_ent" {
				//Seleccionamos las entidades para un user admin concreto
				query2, err := db.Query("SELECT entidades.id, entidades.nombre  FROM entidades INNER JOIN usuarios ON usuarios.entidad_id = entidades.id WHERE usuarios.id = ?", id_user_admin)
				if err != nil {
					Error.Println(err)
				}
				for query2.Next() {
			...
			if internal_action == "entidades" {
				id_entidad := r.FormValue("id_entidad")
				//Seleccionamos el almacen para una entidad concreta
				query3, err := db.Query("SELECT almacenes.id, almacenes.almacen, entidades.nombre FROM almacenes INNER JOIN entidades WHERE entidades.id = almacenes.entidad_id  AND almacenes.entidad_id = ? AND almacenes.creador_id = (SELECT entidad_id FROM usuarios WHERE id = ?)", id_entidad, id_user_admin)
				if err != nil {
					Error.Println(err)
				}
				for query3.Next() {
			...
			if internal_action == "almacenes" {
				id_almacen := r.FormValue("id_almacen")
				//Seleccionamos el pais para un almacen concreto
				query4, err := db.Query("SELECT pais.id, pais.pais, almacenes.almacen FROM pais INNER JOIN almacenes WHERE almacenes.id = pais.almacen_id  AND pais.almacen_id = ? AND pais.creador_id = (SELECT entidad_id FROM usuarios WHERE id = ?)", id_almacen, id_user_admin)
				if err != nil {
					Error.Println(err)
				}
				for query4.Next() {
			...
			if internal_action == "paises" {
				id_pais := r.FormValue("id_pais")
				//Seleccionamos la region para un país concreto
				query5, err := db.Query("SELECT region.id, region.region, pais.pais FROM region INNER JOIN pais ON region.pais_id = pais.id WHERE  region.pais_id = ? AND region.creador_id = (SELECT entidad_id FROM usuarios WHERE id = ?)", id_pais, id_user_admin)
				if err != nil {
					Error.Println(err)
				}
				for query5.Next() {
			...
			if internal_action == "regiones" {
				//Seleccionamos la provincia para una región concreta
				query6, err := db.Query("SELECT provincia.id, provincia.provincia, region.region FROM provincia INNER JOIN region ON provincia.region_id = region.id WHERE provincia.region_id = ? AND provincia.creador_id = (SELECT entidad_id FROM usuarios WHERE id = ?)", r.FormValue("id_region"), id_user_admin)
				if err != nil {
					Error.Println(err)
				}
				for query6.Next() {
			...
			if internal_action == "provincias" {
				//Seleccionamos la tienda para una provincia concreta
				query7, err := db.Query("SELECT tiendas.id, tiendas.tienda, provincia.provincia FROM tiendas INNER JOIN provincia ON tiendas.provincia_id = provincia.id WHERE tiendas.provincia_id = ? AND tiendas.creador_id = (SELECT entidad_id FROM usuarios WHERE id = ?)", r.FormValue("id_provincia"), id_user_admin)
				if err != nil {
					Error.Println(err)
				}
				for query7.Next() {
			...
			if internal_action == "tiendas" {
				//Seleccionamos la tienda para una provincia concreta
				query7, err := db.Query("SELECT tienda FROM tiendas WHERE id = ? AND creador_id = (SELECT entidad_id FROM usuarios WHERE id = ?)", r.FormValue("id_tienda"), id_user_admin)
				if err != nil {
					Error.Println(err)
				}
				for query7.Next() {
