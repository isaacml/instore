...
func alta_users(w http.ResponseWriter, r *http.Request) {
	...
	if ok {
		...
		respuesta := fmt.Sprintf("%s", libs.GenerateFORM(serverext["serverroot"]+"/alta_users.cgi", "user;"+clearUser, "name_user;"+name_user, "pass;"+r.FormValue("pass"), "padre;"+username, "input_entidad;"+r.FormValue("entidad"),
...
func edit_user(w http.ResponseWriter, r *http.Request) {
	...
	if ok {
		...
		clearUser := libs.DeleteSplitsChars(r.FormValue("user"))
		name_user := libs.DeleteSplitsChars(r.FormValue("name_user"))
		respuesta := fmt.Sprintf("%s", libs.GenerateFORM(serverext["serverroot"]+"/edit_user.cgi", "edit_id;"+r.FormValue("id"), "user;"+clearUser, "name_user;"+name_user, "pass;"+r.FormValue("pass"), "entidad;"+r.FormValue("entidad"), "admin_user;"+username,
