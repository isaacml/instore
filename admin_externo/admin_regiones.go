...
func regiones(w http.ResponseWriter, r *http.Request) {
	...
	if ok {
		...
		if accion == "edit_region" {
			//Eliminamos puntos, dos puntos y puntos comas
			correct_res := libs.DeleteSplitsChars(r.FormValue("region"))
			respuesta := fmt.Sprintf("%s", libs.GenerateFORM(serverext["serverroot"]+"/regiones.cgi", "accion;edit_region", "edit_id;"+r.FormValue("id"), "region;"+correct_res, "username;"+username, "pais;"+r.FormValue("id_pais")))
			fmt.Fprint(w, respuesta)
		}
