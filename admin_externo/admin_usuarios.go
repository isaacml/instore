...
func usuarios(w http.ResponseWriter, r *http.Request) {
	...
	if ok {
		...
		if accion == "own_user" {
			...
			respuesta := fmt.Sprintf("%s", libs.GenerateFORM(settings["serverroot"]+"/usuarios.cgi", "accion;own_user", "username;"+clearUser, "old_user;"+username, "password;"+r.FormValue("pass"), "repeat-password;"+r.FormValue("repeat-password")))
			if respuesta == "OK" {
		...
		if accion == "new_user" {
			...
			respuesta := fmt.Sprintf("%s", libs.GenerateFORM(settings["serverroot"]+"/usuarios.cgi", "accion;alta_users", "user;"+clearUser, "name_user;"+name_user, "pass;"+r.FormValue("pass"), "padre;"+username, "input_entidad;"+r.FormValue("entidad"),
		...
		if accion == "get_users" {
			respuesta := fmt.Sprintf("%s", libs.GenerateFORM(settings["serverroot"]+"/usuarios.cgi", "accion;tabla_users", "username;"+username))
			fmt.Fprint(w, respuesta)
		}
		//Envio de datos al server_ext: Cargar datos de un usuario concreto
		if accion == "load_user" {
			respuesta := fmt.Sprintf("%s", libs.GenerateFORM(settings["serverroot"]+"/usuarios.cgi", "accion;load_user", "edit_id;"+r.FormValue("load")))
			respuesta2 := fmt.Sprintf("%s", libs.GenerateFORM(settings["serverroot"]+"/usuarios.cgi", "accion;bitmap_checked", "edit_id;"+r.FormValue("load")))
		...
		if accion == "edit_user" {
			...
			clearUser := libs.DeleteSplitsChars(r.FormValue("user"))
			name_user := libs.DeleteSplitsChars(r.FormValue("name_user"))
			respuesta := fmt.Sprintf("%s", libs.GenerateFORM(settings["serverroot"]+"/usuarios.cgi", "accion;edit_user", "edit_id;"+r.FormValue("id"), "user;"+clearUser, "name_user;"+name_user, "pass;"+r.FormValue("pass"), "entidad;"+r.FormValue("entidad"), "admin_user;"+username,
		...
		if accion == "listEnt" {
			respuesta := fmt.Sprintf("%s", libs.GenerateFORM(settings["serverroot"]+"/usuarios.cgi", "accion;user_entidad", "username;"+username))
			fmt.Fprint(w, respuesta)
		}
	}
}
