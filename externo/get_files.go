...
func get_files(w http.ResponseWriter, r *http.Request) {
	...
	f, err := os.OpenFile(dir_files_location+nameFileServer, os.O_WRONLY|os.O_CREATE, 0666)
	if err != nil {
		Error.Println(err)
		return
	}
	...
	if copy_err != nil {
		...
		for query.Next() {
			...
			db_mu.Unlock()
			//fmt.Printf("INSERT INTO publi (`fichero`, `fecha_inicio`, `fecha_final`, `destino`, `creador_id`, `timestamp`) VALUES (%s,%s,%s,%s,%d,%d)", nameFileServer, r.FormValue("f_inicio"), r.FormValue("f_final"), r.FormValue("destino"), id, timestamp)
			if err1 != nil {
				Error.Println(err1)
			}
		}
